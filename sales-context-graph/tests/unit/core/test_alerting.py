...
threshold checks. Sets the two Gauges to known values before each
...
def test_no_breach_when_both_gauges_are_under_threshold():
    INGESTION_QUEUE_DEPTH.set(5)
    INGESTION_QUEUE_OLDEST_JOB_AGE_SECONDS.set(10)

    breaches = check_gauge_thresholds(max_queue_depth=100, max_oldest_job_age_seconds=900)
...
def test_both_gauges_can_breach_independently():
    INGESTION_QUEUE_DEPTH.set(150)
    INGESTION_QUEUE_OLDEST_JOB_AGE_SECONDS.set(1200)
...
async def test_check_and_alert_returns_breaches_without_a_webhook_configured():
    ...
    breaches = await check_and_alert(None, max_queue_depth=100, max_oldest_job_age_seconds=900)
...
async def test_check_and_alert_posts_to_slack_only_when_there_are_breaches(monkeypatch):
    ...
    INGESTION_QUEUE_DEPTH.set(0)
    INGESTION_QUEUE_OLDEST_JOB_AGE_SECONDS.set(0)
    await check_and_alert("https://hooks.slack.example/x", max_queue_depth=100, max_oldest_job_age_seconds=900)
    assert posted == []

    # Breach -- must post exactly once.
    INGESTION_QUEUE_DEPTH.set(150)
    await check_and_alert("https://hooks.slack.example/x", max_queue_depth=100, max_oldest_job_age_seconds=900)
    assert len(posted) == 1
