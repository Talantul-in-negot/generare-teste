# Ported from ai-knowledge-graph-platform (graphrag/graph/alias_registry.py) — domain-agnostic as-is, unmodified except where noted.
...
# TODO(sales-context-graph): this whole regulatory-prefix block (and its two
# call sites below, search _normalize_regulatory / _REGULATION_TYPES) is
# aerospace-domain debris from the source platform. Delete it, or replace
# with a sales-domain equivalent if one turns out to be needed (e.g.
# stripping "Inc."/"Ltd"/"GmbH" company-suffix noise from account names —
# a real but different normalization problem, don't reuse this regex for it).
#
