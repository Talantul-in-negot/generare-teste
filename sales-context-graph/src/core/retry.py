# Forked from ai-knowledge-graph-platform (graphrag/core/retry.py) — verbatim, no graphrag.* imports to fix.

"""Async retry decorator for transient infrastructure failures.

Problem solved
--------------
Neo4j (and other infra) calls can fail transiently (connection resets,
leader elections, rate limits).  Without retries these failures surface
as 500s to the user even though a second attempt would succeed.
