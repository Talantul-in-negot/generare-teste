...
class ConversationSummary(BaseModel):
    ...
    rollup for "what happened on this call"-shaped questions, alongside (not
    instead of) the micro-level Claims src/context_graph/builder.py already
    retrieves. Generated by src/summarization/call_summary.py, which reuses
    NarrativeSummaryUseCase's already-tested grounded-citation pipeline
    (src/narrative/grounding.py) rather than a parallel one -- every id in
    cited_claim_ids is a real Claim.claim_id that was actually supplied to
    the model, verified the same way every other narrative in this repo is.
    One per Conversation (MERGE key is conversation_id, see
    ConversationRepository.upsert_conversation_summary); regenerating
    replaces the prior summary rather than accumulating a history, since
    this is a derived view, not itself evidence."""
    conversation_id: str
    workspace_id: str
    text: str
