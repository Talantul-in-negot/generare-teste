...
| JD Requirement | Project Evidence | File / Endpoint | Demo Step | Gap |
|---|---|---|---|---|
| Production Python | 22,049 LOC (106 files), fully async, retry/backoff, structured logging | `graphrag/` package | — | None |
| Test suite | 541 passing unit tests, 6,322 test LOC across 36 files | `tests/unit/` → `make test` | — | Integration test coverage is lighter than unit |
...
| Agentic retrieval | IRCoT: Groq 8B routing + DeepSeek synthesis, max 2 steps | `graphrag/retrieval/agentic_retriever.py` | — | None |
...
| Provider choice | ADR-0004: DeepSeek for large generation, Groq for fast routing/fallback, OpenAI for embeddings | `docs/adr/0004-groq-over-gemini-for-text-generation.md` | — | None |
...
| Contradiction detection | 4 conflict types; surfaced per tenant | `graphrag/graph/contradiction_detector.py` | Demo step 5 | None |
...
| Claim | How to verify |
|---|---|
| "Not a tutorial project" | `make test` → 541 passing; `make smoke-test` → exits 0 |
...
| Open conflicts detected | ⚠ stale — this predates the retirement of the `multi_source` strategy, which had been miscounting corroborating sources as conflicts. Read the current count off the Admin dashboard → Conflicts tab. | Admin dashboard → Conflicts tab |
...
| Passing tests | 541 | `python -m pytest tests/unit -q` |
| KG modules | 43 | `ls graphrag/graph/*.py \| wc -l` |
| ADRs | 6 | `ls docs/adr/*.md` |
| Lines of code | 22,049 (graphrag/, 106 files) + 6,322 (tests/unit/, 36 files) | — |
