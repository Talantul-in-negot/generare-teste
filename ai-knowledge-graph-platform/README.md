...
| **RDF / OWL / SKOS export** | `scripts/export_rdf.py` serialises to Turtle with `owl:NamedIndividual`, `owl:ObjectProperty`, `rdfs:subClassOf`, reified confidence annotations, and tenant-scoped `skos:ConceptScheme` navigation; `--infer` applies OWL-RL closure before writing |
